...
class IterateKernel {
 ...
 protected:
  void Init(const CommSpec& comm_spec, const FRAG_T& frag, bool dependent,
    ...
    auto all_inner_vertices = frag.AllInnerVertices(); //inner + outer master
    auto inner_vertices = frag.InnerVertices(); //inner vertices
    auto master_vertices = frag.MasterVertices(); //innermaster - outer master

    values_.Init(all_inner_vertices);   //values是只有参与计算的顶点拥有
    deltas_.Init(vertices, default_v());  //delta 是所有的点都有的，包括内部点和外部的虚拟点
    master_deltas_.Init(master_vertices,default_v()); //master_delta是本地master和outer master所拥有的。
    send_master_deltas_.Init(master_vertices, default_v()); //用于发送master的delta值，local master 和outer master拥有
    extra_delta_.Init(master_vertices,default_v());
    send_deltas_.Init(vertices, default_v()); //用于发送outer的delta值。
    if (data_driven) {
  ...
  VertexArray<value_t, vid_t> extra_delta_{};
  VertexArray<value_t, vid_t> master_deltas_{};
  VertexArray<value_t, vid_t> send_master_deltas_{};
  VertexArray<value_t, vid_t> send_deltas_{};
  ...
  template <typename APP_T>
  friend class IngressSyncDelayIterWorker;
};
