...
class TraversalAppBase {
 public:
  ...
  virtual void Compute(const vertex_t& u, const value_t& value,
                       ...
                       DenseVertexSet<vid_t>& send_modified) = 0;
 ...
 protected:
  void Init(const CommSpec& comm_spec, std::shared_ptr<FRAG_T>& frag) {
    ...
    memory += send_modified_.Range().size() / 64;
  ...
  DenseVertexSet<vid_t> curr_modified_, next_modified_,send_modified_;
