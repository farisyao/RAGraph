...
ParallelEngineSpec MultiProcessSpec(const CommSpec& comm_spec,
                                    bool affinity = false) {
  ParallelEngineSpec spec;
...
class ParallelEngine {
 public:
   ...
   */
  template <typename ITER_FUNC_T, typename VID_T>
  inline void ForEachSimple(const DenseVertexSet<VID_T>& dense_set,
                            const VertexRange<VID_T>& range,
                            const ITER_FUNC_T& iter_func,
                            int chunk_size = 1024) {
    std::vector<std::thread> threads(thread_num_);
    auto end = range.end().GetValue();
    std::atomic<VID_T> cur(range.begin().GetValue());

    for (uint32_t i = 0; i < thread_num_; ++i) {
      threads[i] = std::thread(
          [chunk_size, &iter_func, &dense_set, &cur, end](uint32_t tid) {
            while (true) {
  ...
  template <typename ITER_FUNC_T, typename VID_T>
  inline void ForEachCilk(const DenseVertexSet<VID_T>& dense_set,
                          const VertexRange<VID_T>& range,
                          const ITER_FUNC_T& iter_func) {
#ifdef INTERNAL_PARALLEL
    LOG(FATAL) << "Cilk is not enabled";
#endif
    parallel_for(auto i = range.begin().GetValue(); i < range.end().GetValue();
                 i++) {
      Vertex<VID_T> v(i);
    ...
    template <typename ITER_FUNC_T, typename VID_T>
  inline void ForEachCilkOfBitset(const DenseVertexSet<VID_T>& dense_set,
                          const VertexRange<VID_T>& range,
                          const ITER_FUNC_T& iter_func, 
                          int chunk_size = 1024) {
#ifdef INTERNAL_PARALLEL
    ...
    auto& bitset = dense_set.GetBitset();
    parallel_for (VID_T vid = batch_begin; vid < batch_end; vid += 64) {
      Vertex<VID_T> v(vid);
      uint64_t word = bitset.get_word(vid - offset);
      // int thread_id = __cilkrts_get_worker_number(); // slow
