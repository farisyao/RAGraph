...
class ThreadLocalMessageBuffer {
 public:
  ...
  inline void SyncStateOnOuterVertex(const GRAPH_T& frag,
    ...
    //LOG(INFO)<<"...............v:"<< frag.GetOuterVertexGid(v);
  ...
  inline void SendMsgThroughOEdges(const GRAPH_T& frag,
    ...
    /**
   * @brief Communication via a crossing sync mirror a->a. It sends message
   * from a to b.
   *
   * @tparam GRAPH_T Graph type.
   * @tparam MESSAGE_T Message type.
   ...
   */
  template <typename GRAPH_T, typename MESSAGE_T>
  inline void SendMsgThroughFid(const GRAPH_T& frag,
                                   const typename GRAPH_T::vertex_t& v,
                                   const MESSAGE_T& msg) {
    if (frag.IsInnerVertex(v)) {
      DestList dsts = frag.MasterFidList(v);
      if (dsts.Empty()){
        return;
      }
      fid_t* ptr = dsts.begin;
      typename GRAPH_T::vid_t gid = frag.GetMasterVertexGid(v);
    
      //std::string s = std::to_string(frag.fid())+ "gid: " + std::to_string(gid) + " msg:"+std::to_string(msg)+" fid: ";
      //LOG(INFO)<<frag.fid()<<"++++++++++++++++++++++++++++++"<<gid<<":"<<msg;
      while (ptr != dsts.end) {
        fid_t fid = *(ptr++);
        //s += std::to_string(fid) +" ";
        //if (gid == 0) 
        //LOG(INFO)<<gid<<": "<<msg<<" :"<<frag.fid()<<"dsts"<<fid<<" size: ";
        to_send_[fid] << gid << msg;
        if (to_send_[fid].GetSize() > block_size_) {
          //sent_size_ += to_send_[fid].GetSize();
          flushLocalBuffer(fid);
      ...
      fid_t fid = frag.GetFragId(v);
      to_send_[fid] << frag.GetMasterVertexGid(v) << msg;
      if (to_send_[fid].GetSize() > block_size_) {
        //sent_size_ += to_send_[fid].GetSize();
        flushLocalBuffer(fid);
      }
