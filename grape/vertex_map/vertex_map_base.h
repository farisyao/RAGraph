...
class VertexMapBase {
 ...
 public:
  ...
  virtual size_t GetTotalMasterVertexSize() = 0;
  virtual size_t GetInnerMasterVertexSize(fid_t fid) = 0;


  // for constructing the vertexmap.
  virtual void Clear() = 0;
  virtual void AddLocalVertexAndDegree(const OID_T& oid, const uint32_t degree, const uint32_t outdgree) = 0;
  virtual void AddVertex(fid_t fid, const OID_T& oid) = 0;
  virtual bool AddVertex(fid_t fid, const OID_T& oid, VID_T& gid,const bool issrc) = 0;
  virtual void Construct() = 0;
  virtual void Construct_master() = 0;
  virtual void GetMasterHash(std::unordered_set<VID_T>& IsMaster_) = 0;
  virtual void GetMasterHash2Degree(HashMap<VID_T, uint32_t>& g2degree) = 0;
  ...
  //virtual bool GetDegree(fid_t fid, const VID_T& lid, uint32_t& degree) = 0;
};
