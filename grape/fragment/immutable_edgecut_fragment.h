...
class ImmutableEdgecutFragment
 ...
 public:
  ...
  virtual ~ImmutableEdgecutFragment() {
    LOG(INFO) << "ImmutableEdgecutFragment is freed";
  }

  void Init(fid_t fid, std::vector<internal_vertex_t>& vertices,
            std::vector<edge_t>& edges) override {  //local edges:edges:gidsrc,giddes,edata
    std::unordered_set<VID_T> IsMaster_;
    fid_ = fid;
    fnum_ = vm_ptr_->GetFragmentNum();
    vm_ptr_->GetMasterHash(IsMaster_);
    // for (auto i : IsMaster_) {
    //   LOG(INFO)<<fid_<<":"<<i;
    // }
    //fid_offset为移动的位数 gid : 二进制，最前n位是fid，后边是local id 二进制。 id_mask 为 32-n位的1
    calcFidBitWidth(fnum_, id_mask_, fid_offset_);

    //ivnum_ contains imvnum, and the master is ranked at the end of ivnum
    ivnum_ = vm_ptr_->GetInnerVertexSize(fid_); //l2o[fid].size
    imvnum_ = vm_ptr_->GetInnerMasterVertexSize(fid_);  //master_l2g_[fid].size
    ...
    auto is_master_gid = [IsMaster_](VID_T id) { return IsMaster_.find(id) != IsMaster_.end(); };
   
   
    //find inner vertices and outer vetices
    //only modify LoadStrategy::kOnlyOut for multi-master
    {
      std::vector<VID_T> outer_vertices;
      std::vector<VID_T> master_vertices;  //outer master vertices
      auto first_iter_in = [&is_iv_gid, invalid_vid](
                               Edge<VID_T, EDATA_T>& e,
                               std::vector<VID_T>& outer_vertices) {
        if (is_iv_gid(e.dst_)) {
          if (!is_iv_gid(e.src_)) {
            outer_vertices.emplace_back(e.src_);
          }
        } else {
          e.src_ = invalid_vid;
        }
      };
      auto first_iter_out = [this, &is_iv_gid, &is_master_gid, invalid_vid](
                                Edge<VID_T, EDATA_T>& e,
                                std::vector<VID_T>& outer_vertices,
                                std::vector<VID_T>& master_vertices) {
       
        if (is_iv_gid(e.src_)) { 
          if (!is_master_gid(e.src_)) { //inner local -> outer local  /innner local -> outer master
            if (!is_iv_gid(e.dst_) ) {
              if (is_master_gid(e.dst_))
                master_vertices.emplace_back(e.dst_);
              else
                outer_vertices.emplace_back(e.dst_);
            }
          } else { //inner master -> outer local  // inner master -> outer master
            ...
            //   }
            // }
        } else { //outer src -> inner dst:
          if (is_master_gid(e.src_)) { //outer master src
            master_vertices.emplace_back(e.src_);
          }
      ...
      if (load_strategy == LoadStrategy::kOnlyIn) {
        ...
        for (auto& e : edges) {
          first_iter_out(e, outer_vertices, master_vertices);
        }
      ...
      DistinctSort(outer_vertices); //sort以后可以按frag分好
      DistinctSort(master_vertices); //sortmaster顶点,以按fragment排列

      ovgid_.resize(outer_vertices.size());
      memcpy(&ovgid_[0], &outer_vertices[0],
             outer_vertices.size() * sizeof(VID_T));

      mvgid_.resize(master_vertices.size());
      memcpy(&mvgid_[0], &master_vertices[0],
            master_vertices.size() * sizeof(VID_T)); 
     
    }
 
    tvnum_ = ivnum_; //total vnum = invnum  + omvnvm + onvm
    for (auto gid : mvgid_) { 
      mvg2l_.emplace(gid,tvnum_);
      ++tvnum_;
    }
    omvnum_ = tvnum_ - ivnum_;
    for (auto gid : ovgid_) {
      ovg2l_.emplace(gid, tvnum_);
      ++tvnum_;
    }
    
    ovnum_ = tvnum_ - ivnum_ - omvnum_;
    //init idegree[local id],odegree[local id],Preprocessing local vertices by adding edges 
    {
      ...
      auto mv_gid_to_lid = [this](VID_T gid) { return mvg2l_.at(gid); };
      ...
      auto second_iter_out = [this, &iv_gid_to_lid, &ov_gid_to_lid, &mv_gid_to_lid, invalid_vid,
                              &is_iv_gid, &is_master_gid](Edge<VID_T, EDATA_T>& e,
                                          std::vector<int>& idegree,
                                          std::vector<int>& odegree) {    
        
          // if (e.src() == 1073741827 && e.dst() == 1073741824) {
            
          //   LOG(INFO)<<fid_<<"++++++++"<<e.src()<<":::"<<e.dst();
          // }
        // if (fid_ == 1)
        //        LOG(INFO)<<fid_<<",,,,,,,"<<e.src()<<":::"<<e.dst();
        if (is_iv_gid(e.src_)) {
          if (is_master_gid(e.src_)) { //inner-master顶点 目的顶点:inner顶点
            VID_T s = e.src();
            e.src_ = iv_gid_to_lid(e.src_);
            if (is_iv_gid(e.dst_)) {
              VID_T d= e.dst();
              ...
              ++odegree[e.src_];
              ++oenum_; 
              // if (s == 1073741827 && d == 1073741824)
              //  LOG(INFO)<<fid_<<",,,,,,,"<<e.src()<<":::"<<e.dst();
            } else {    //如果dst不是inner v，放入到idegree,用来后边找master顶点副本点.这里需要将入边 出边都考虑进来
              //if (is_master_gid(e.dst_)) {
                 //e.dst_ = mv_gid_to_lid(e.dst_);
                 
                 fid_t ifid = e.dst_ >> fid_offset_;
                 ineighborfid_[e.src_].emplace_back(ifid);
                 e.src_ = invalid_vid;
                //  if (is_master_gid(e.dst_))
                //   e.dst_ = mv_gid_to_lid(e.dst_);
                //   else
          ...
          } else { //inner-local顶点, 目的顶点:所有类型的顶点
            e.src_ = iv_gid_to_lid(e.src_);
             if (is_iv_gid(e.dst_)) {
                e.dst_ = iv_gid_to_lid(e.dst_);
              } else if (is_master_gid(e.dst_)){
                e.dst_ = mv_gid_to_lid(e.dst_);
              } else {
                e.dst_ = ov_gid_to_lid(e.dst_);
              }
        ...
        } else if (is_master_gid(e.src_)){  //outer- master/因为src如果是个外部点的话,des一定是inner点
          e.src_ = mv_gid_to_lid(e.src_);
          e.dst_ = iv_gid_to_lid(e.dst_);
          ++odegree[e.src_];
          ++oenum_;
        } else {  //src是外部非master顶点(outer vertex),dst一定是本地的inner顶点
          // if (is_master_gid(e.dst_)) { //已知dst一定是inner顶点，所以只要限制是master顶点即可
          //    if (fid_ == 1) {
          //   LOG(INFO)<<"src:"<<e.src_<<"dst:"<<e.dst_;
          // }
          //   e.src_ = ov_gid_to_lid(e.src_);
          //   e.dst_ = mv_gid_to_lid(e.dst_);
          //   ++idegree[e.dst_];
          ...
          // } else {
            if (is_master_gid(e.dst_)) {
              fid_t ifid = e.src_ >> fid_offset_;
              e.dst_ = iv_gid_to_lid(e.dst_);
              ineighborfid_[e.dst_].emplace_back(ifid);
              e.src_ = invalid_vid;
            }
            if (!is_master_gid(e.dst_))
              e.src_ = invalid_vid;
    ...
    //现在oeoffset中存储着计算所需的邻居顶点信息。ieoffset中存储的是远程顶点信息，用于之后获取fids使用。
      ie_.resize(ienum_);
      oe_.resize(oenum_);
      ...
      // for (VID_T i = 0; i < tvnum_; ++i) {
      //   vertex_t u(i);
      //   LOG(INFO)<<fid_<<"==========="<<this->Vertex2Gid(u)<<":"<<odegree[i];
      // }
    }
...
//根据odgree和idegree向eoffset中添加邻居顶点id。
      auto third_iter_out = [invalid_vid, this](
                                const Edge<VID_T, EDATA_T>& e,
                                Array<nbr_t*, Allocator<nbr_t*>>& ieiter,
                                Array<nbr_t*, Allocator<nbr_t*>>& oeiter) {
    
        if (e.src_ != invalid_vid) { //去掉outer->inner-local的边
          if (e.src_ < (ivnum_ - imvnum_)) {  //inner-local的src,所有的dst顶点都留下来
            oeiter[e.src_]->GetEdgeDst(e);
            ++oeiter[e.src_];
          } else if (((ivnum_ - imvnum_) <= e.src_) && (e.src_ < ivnum_)) { //src is inner-master
            if (e.dst_ < ivnum_) {
              oeiter[e.src_]->GetEdgeDst(e);
              ++oeiter[e.src_];
            }
            //if (e.dst_ >= (tvnum_ - ovnum_)) {
            // else {
            //   ieiter[e.src_]->GetEdgeDst(e);
            //   ++ieiter[e.src_];
            // }
          } else if ((e.src_ >= ivnum_) && (e.src_ < (tvnum_ - ovnum_))) { //outer-master将消息传递给本地的邻居
            if (e.dst_ < ivnum_) {
              oeiter[e.src_]->GetEdgeDst(e);
              ++oeiter[e.src_];
            }
          } else { // invalid_vid限制了src :outer ,dst :local-inner ,所以该边dst一定是inner-master
              // ieiter[e.dst_]->GetEdgeDst(e);
              // ++ieiter[e.dst_];
          }
        }
      };
    ...
    // auto range = this->InnerVertices();
    //  for (auto v : range) {
    //     std::string s = "gid:";
    //     vid_t vgid = this->Vertex2Gid(v);
    //     s += std::to_string(vgid)+" ies:";
    //     auto ies = this->GetIncomingAdjList(v);
    //     auto oes = this->GetOutgoingAdjList(v);
    //     for (auto& n : ies) {
    //       vertex_t lid = n.neighbor;
    //       vid_t gid = this->Vertex2Gid(lid);
    //       s += std::to_string(gid) + " ";
    //     }
    //     s += " oes:";
    //     for (auto& m : oes) {
    //       vertex_t lid = m.neighbor;
    //       vid_t gid = this->Vertex2Gid(lid);
    //       s += std::to_string(gid) + " ";
    //     }
    //    LOG(INFO) << "[fid] " << fid_ <<": "<<s;
    //  }
    ///////////////////////////////////////////////////////
    initOuterVerticesOfFragment(); //frag_v_num and find lid range of every fragment
    
    initLocalVerticesDegree(); //degree of local vertices
    ...
    if (sizeof(internal_vertex_t) > sizeof(VID_T)) {
      for (auto& v : vertices) {
        ...
        if (gid >> fid_offset_ == fid_) {
          ...
          auto it = mvg2l_.find(gid);
          if (it != mvg2l_.end()) {
            vdata_[it->second] = v.vdata();
          }
        }
      }
    }
  ...
  inline size_t GetMasterVerticesNum() const override {return imvnum_ + omvnum_; }
  ...
  inline VertexRange<VID_T> InnerLocalVertices() const override {
    return VertexRange<VID_T>(0, ivnum_ - imvnum_);
  }
  ...
  inline VertexRange<VID_T> OuterVertices() const override {
    return VertexRange<VID_T>(ivnum_ + omvnum_, tvnum_);
  }

  inline VertexRange<VID_T>InnerMasterVertices() const override {
    return VertexRange<VID_T>((ivnum_ - imvnum_), ivnum_);
  }

  inline VertexRange<VID_T>OuterMasterVertices() const override {
    return VertexRange<VID_T>(ivnum_, ivnum_ + omvnum_);
  }

  inline VertexRange<VID_T>MasterVertices() const override {
    return VertexRange<VID_T>(ivnum_ - imvnum_, ivnum_+omvnum_);
  }
  ...
  inline VertexRange<VID_T> AllInnerVertices() const override {
    return VertexRange<VID_T>(0, ivnum_ + omvnum_);
  }
  ...
  inline OID_T GetId(const vertex_t& v) const override {
    return IsInnerVertex(v) ? GetInnerVertexId(v) : IsOuterVertex(v) ?  GetOuterVertexId(v) : GetOuterMasterVertexId(v);
  }
  
 inline VertexRange<VID_T> BoundVertices() const override {
    return VertexRange<VID_T>(ivnum_ - imvnum_ , tvnum_);
  }

  //获取outer master and outer的fid
  inline fid_t GetFragId(const vertex_t& u) const override {
    return IsOuterVertex(u)
               ? (fid_t)(ovgid_[u.GetValue() - ivnum_ - omvnum_] >> fid_offset_)
               : (fid_t)(mvgid_[u.GetValue() - ivnum_] >> fid_offset_);
  }
  ...
  inline int GetLocalDegree(const vertex_t& v) const override {
    assert(IsLocalVertex(v));
    return l2degree_[v.GetValue()];
  }

  inline bool IsInnerGid(const VID_T gid) const {
    return (gid >> fid_offset_) == fid_;
  }
  ...
  inline VID_T Vertex2Gid(const vertex_t& v) const override {
    return IsInnerVertex(v) ? GetInnerVertexGid(v) : IsOuterVertex(v) ? GetOuterVertexGid(v) : GetOuterMasterVertexGId(v);
  }
  ...
  inline bool IsOuterVertex(const vertex_t& v) const override {
    return (v.GetValue() < tvnum_ && v.GetValue() >= (ivnum_ + omvnum_));
  }

  inline bool IsMasterVertex(const vertex_t& v) const override {
    return (v.GetValue() >= (ivnum_ - imvnum_) && v.GetValue() < tvnum_ - ovnum_);
  }

  inline bool IsLocalVertex(const vertex_t& v) const override {
    return (v.GetValue() < ivnum_ + omvnum_);
  }
  ...
  inline OID_T GetOuterVertexId(const vertex_t& v) const override {
    VID_T gid = ovgid_[v.GetValue() - ivnum_ - omvnum_];
    OID_T internal_oid;
    vm_ptr_->GetOid(gid, internal_oid);
    return internal_oid;
  }

  inline OID_T GetOuterMasterVertexId(const vertex_t& v) const override {
    VID_T gid = mvgid_[v.GetValue() - ivnum_];
    OID_T internal_oid;
    vm_ptr_->GetOid(gid, internal_oid);
    return internal_oid;
  }
  ...
  std::shared_ptr<vertex_map_t> vm_ptr() const {
    return vm_ptr_;
  }
  
  VertexArray<edata_t, vid_t> weight_sum() const {
    return weight_sum_;
  }
  ...
  inline bool OuterVertexGid2Vertex(const VID_T& gid,
                                    vertex_t& v) const override {
    auto iterm = mvg2l_.find(gid);
    auto itero = ovg2l_.find(gid);

    if (iterm != mvg2l_.end()) {
      v.SetValue(iterm->second);
      return true;
    }else if (itero != ovg2l_.end()) {
      v.SetValue(itero->second);
      return true;
    } else {
      return false;
    }
  ...
  inline bool MasterVertexGid2Vertex(const VID_T& gid,
                                     vertex_t& v) const override {
    if ((gid >> fid_offset_) == fid_) { //inner master
      v.SetValue(gid & id_mask_);
    } else { //outer master
      auto iter = mvg2l_.find(gid);
      if (iter != mvg2l_.end()) {
  ...
  inline VID_T GetOuterMasterVertexGId (const vertex_t& v) const override {
    return mvgid_[v.GetValue() - ivnum_];
  }
  inline VID_T GetOuterVertexGid(const vertex_t& v) const override {
    return ovgid_[v.GetValue() - ivnum_ - omvnum_];
  }
  inline VID_T GetInnerVertexGid(const vertex_t& v) const override {
    return (v.GetValue() | ((VID_T) fid_ << fid_offset_));
  }
  inline VID_T GetMasterVertexGid (const vertex_t& v)  const override {
    return IsInnerVertex(v) ? GetInnerVertexGid(v) : GetOuterMasterVertexGId(v);
  }
  ...
  inline DestList MasterFidList(const vertex_t& v) const override {
    assert(!odoffset_.empty());
    assert(IsMasterVertex(v));
    vid_t inner = ivnum_ - imvnum_;
    return DestList(odoffset_[v.GetValue() - inner], odoffset_[v.GetValue() - inner + 1]);
  }
 ...
 public:
  ...
  inline  std::vector<fid_t>& getMasterInFid(VID_T gid)  {
    return ineighborfid_[gid];
  }
  ...
  void SetUpWeightSum(HashMap<vid_t,edata_t>& count_weight) {
    auto range = AllInnerVertices();
    weight_sum_.Init(range, 0);
    for (auto v : range) {
      vid_t gid = Vertex2Gid(v);
      weight_sum_[v] = count_weight[gid];
    }
  }
  ...
  void SetupMirrorInfo(fid_t fid, const VertexRange<VID_T>& range,
    ...
    for (size_t i = 0; i < gid_list.size(); ++i) {
      ...
      vertex_vec[i].SetValue(gid_list[i] & id_mask_); //ser mirrors lid
    }
  }
  void SetUpMasterFidInfo(HashMap<vid_t, std::vector<fid_t>>& fid_list) {
    odst_.clear();
    odoffset_.clear();
    std::vector<fid_t> tmp_fids;
    size_t mnum = GetMasterVerticesNum();//？？？？？？
    std::vector<int> id_num(mnum, 0);
    for (int i = 0; i < mnum; i++) {
      vertex_t u(ivnum_ - imvnum_ + i);
      vid_t gid = Vertex2Gid(u);
      if (fid_list[gid].size() != 0) {
        id_num[i] = fid_list[gid].size()-1; //去掉本地fid,方便通信
        for (auto fid : fid_list[gid]) {
          if (fid != fid_){
    ...
    //   LOG(INFO)<<fid_<<"ddddd"<<id_num[index];
    
    // for (int i = 0; i < id_num[index]; i++) {
    //   LOG(INFO)<<fid_<<"ssssss"<<tmp_fids[index+i];
    // }
    odst_.resize(tmp_fids.size());
    odoffset_.resize(mnum + 1);
    memcpy(&odst_[0], &tmp_fids[0], sizeof(fid_t) * odst_.size());
    odoffset_[0] = odst_.data();  //
    for (VID_T i = 0; i < mnum; ++i) {
      odoffset_[i + 1] = odoffset_[i] + id_num[i];
    }

    // if (fid_ == 2){
    //   fid_t* ptr =  odoffset_[0];
    //   for (int i = 0; i < tmp_fids.size(); i++) {
    //       fid_t f = *(ptr);
    //       if (f != tmp_fids[i]) {
    //         LOG(INFO)<<"!!!error";
    //       }
    //       if (ptr == odoffset_[285753]) {
    //         LOG(INFO)<<fid_<<"ssss"<<"dasdasdasd"<<f<<tmp_fids[i]<<"oooo"<<*ptr;
    //       }
    //       ptr++;
    //   }
    ...
    //     fid_t* ptr =  odoffset_[692560];
    //     LOG(INFO)<<fid_<<"ssss"<<"dasdasdasd"<<*(ptr);
 ...
 private:
  ...
  void initOuterVerticesOfFragment() {
    ...
    fid_t cur_fid = 0;
    for (VID_T i = 0; i < ovnum_; ++i) {  //因为进行排序以后，由于gid=fid+lid,所以可以按fid划分开
    ...
    VID_T cur_lid = ivnum_ + omvnum_;
    for (fid_t i = 0; i < fnum_; ++i) {
      VID_T next_lid = cur_lid + frag_v_num[i];
      outer_vertices_of_frag_.emplace_back(cur_lid, next_lid); //find lid range of every fragment
      cur_lid = next_lid;
    }
    CHECK_EQ(cur_lid, tvnum_);
  }

  void initLocalVerticesDegree() {
    ParallelEngine pl;
    HashMap<VID_T, uint32_t> g2degree_;
    vm_ptr_->GetMasterHash2Degree(g2degree_);
    VID_T localindex = ivnum_ + omvnum_;
    l2degree_.resize(localindex);
    VertexRange<VID_T> vertices(0, localindex);
    if (FLAGS_cilk) {
#ifdef INTERNAL_PARALLEL
      LOG(FATAL) << "Ingress is not compiled with -DUSE_CILK";
#endif
      parallel_for(vid_t i = 0; i < localindex; i++) {
        vertex_t u(i);
        if (i < (ivnum_ - imvnum_)) {
          l2degree_[i] = GetLocalOutDegree(u);
        } else {
          if (i < ivnum_ && i >= (ivnum_ - imvnum_)) {
            vid_t gid = GetInnerVertexGid(u);
            l2degree_[i] = g2degree_[gid];
          } else {
            vid_t gid = mvgid_[i - ivnum_];
            l2degree_[i] = g2degree_[gid];
          }
        }
      }
       ...
       pl.ForEach(vertices,
              [this, &g2degree_](int tid, vertex_t u) {
              if (u.GetValue() < (ivnum_ - imvnum_)) {
                l2degree_[u.GetValue()] = GetLocalOutDegree(u);
              } else {
                if (u.GetValue() < ivnum_ && u.GetValue() >= (ivnum_ - imvnum_)) {
                  vid_t gid = GetInnerVertexGid(u);
                  l2degree_[u.GetValue()] = g2degree_[gid];
                } else {
                  vid_t gid = mvgid_[u.GetValue() - ivnum_];
                  l2degree_[u.GetValue()] = g2degree_[gid];
                }
              }
            });
    ...
    // if (fid_ == 0) {
    //  for (int i = 0; i < l2degree_.size(); i++) {
    //    LOG(INFO)<<"fid:"<<fid_<<" i:" <<i <<" degree:" <<l2degree_[i];
    //  }
    // }
  ...
  VID_T ivnum_, imvnum_, ovnum_, omvnum_, tvnum_, id_mask_;
  size_t ienum_{}, oenum_{};
  int fid_offset_{};
  fid_t fid_{}, fnum_{};
  ska::flat_hash_map<VID_T,std::vector<fid_t>> ineighborfid_;
  ska::flat_hash_map<VID_T, VID_T> ovg2l_, mvg2l_;
  Array<VID_T, Allocator<VID_T>> ovgid_, mvgid_;
  ...
  std::vector<uint32_t> l2degree_; //get outdegree for pagerank etc
  VertexArray<edata_t, vid_t> weight_sum_;
