...
class EdgecutFragmentBase
 ...
 public:
  ...
  virtual VertexRange<VID_T> OuterVertices() const = 0;
   ...
   /**
   * @brief Returns the vertex range of inner local vertices in this fragment.
   * 
   * @return The vertex range of inner local vertices in this fragment.
  */
  virtual VertexRange<VID_T>InnerLocalVertices() const = 0;
  /**
   * @brief Returns the vertex range of inner master vertices in this fragment.
   * 
   * @return The vertex range of inner master vertices in this fragment.
  */
  virtual VertexRange<VID_T>InnerMasterVertices() const = 0;

  /**
   * @brief Returns the vertex range of outer master vertices in this fragment.
   * 
   * @return The vertex range of outer master vertices in this fragment.
  */
  virtual VertexRange<VID_T>OuterMasterVertices() const = 0;

  /**
   * @brief Returns the vertex range of master vertices in this fragment.
   * 
   * @return The vertex range of master vertices in this fragment.
  */
  virtual VertexRange<VID_T>MasterVertices() const = 0;

    /**
   * @brief Returns the vertex range of all inner vertices in this fragment.
   *
   * @return The vertex range of all inner vertices in this fragment.
   */
  virtual VertexRange<VID_T> AllInnerVertices() const = 0;

    /* @brief Returns the vertex range of all bound of vertices in this fragment.
   *
   * @return The vertex range of all bound of vertices in this fragment.
   */
  virtual VertexRange<VID_T> BoundVertices() const = 0;
  ...
  /**
   * @brief Check if vertex v is master vertex of this fragment.
   *
   * @param v Input vertex.
   *
   * @return True if vertex v is master vertex, false otherwise.
   */
  virtual bool IsMasterVertex(const vertex_t& v) const = 0;

  /**
   * @brief Check if vertex v is local vertex of this fragment,
   * include all inner vertex and outer master vertex.
   *
   * @param v Input vertex.
   *
   * @return True if vertex v is local vertex, false otherwise.
   */
  virtual bool IsLocalVertex(const vertex_t& v) const = 0;
  ...
  virtual OID_T GetOuterVertexId(const Vertex<VID_T>& v) const = 0;
   ...
   /**
   * @brief Get the original ID of an outer master vertex.
   *
   * @param v Input vertex.
   *
   * @return The original ID.
   */
  virtual OID_T GetOuterMasterVertexId(const Vertex<VID_T>& v) const = 0;
  ...
  virtual bool MasterVertexGid2Vertex(const VID_T& gid,
                                     vertex_t& v) const = 0;

  /**
   * @brief Convert from outer-master vertex handle to its global id.
   *
   * @param v Input vertex handle.
   *
   * @return Global id of the vertex.
   */
  virtual VID_T GetOuterMasterVertexGId (const vertex_t& v) const = 0;
   ...
   * @return Global id of the vertex.
   */
  virtual VID_T GetMasterVertexGid (const vertex_t& v) const = 0;
  /**
  ...
  virtual DestList IOEDests(const Vertex<VID_T>& v) const = 0;
    ...
    /**
   * @brief Return the master fragment ID list of a master vertex.
   *
   * @note: The master vertex obtains the fragment is where its mirror vertex is 
   * located through the fidlist, and then synchronizes the message
   * @note: This method is encapsulated in the corresponding sending message
   * API, SendMsgThroughEdges, so it is not recommended to use this method
   * directly in application programs.
   *
   * @param v Input vertex.
   *
   * @return The master fragment ID list.
   */
  virtual DestList MasterFidList(const vertex_t& v) const = 0;
  /**
