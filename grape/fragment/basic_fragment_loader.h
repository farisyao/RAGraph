...
class BasicFragmentLoader<
 ...
 public:
  ...
  void AddVertex(const oid_t& id, const EmptyType& data, const bool issrc) {
    fid_t fid = partitioner_.GetPartitionId(id);
    vid_t gid;

    vm_ptr_->AddVertex(fid, id, gid, issrc);
  }
  ...
  void ConstructFragment(std::shared_ptr<fragment_t>& fragment) {
    ...
    vm_ptr_->Construct(); //得到全局的o-l id 
   
    VLOG(1) << "[worker-" << comm_spec_.worker_id()
            << "]: finished construct vertex map and process vertices";

    processEdges(); //得到全局gid的本地边消息 processed_edges_ (边的两个顶点至少有一个是本地顶点)
    vm_ptr_->Construct_master(); //构建全局的multi-master顶点
    ...
    fragment = std::shared_ptr<fragment_t>(new fragment_t(vm_ptr_));
    if (FLAGS_application == "php") {
      if constexpr (!std::is_same<edata_t, EmptyType>::value) {
        initWeightSum(fragment, processed_edges_);
        ...
        fragment->SetUpWeightSum(weight_sum);
        weight_sum.clear();
      }
    } else {
      std::vector<internal::Vertex<vid_t, EmptyType>> fake_vertices;
 ...
 private:
  ...
  void processEdges() {
    fid_t _fid = comm_spec_.fid();
    std::vector<work_unit> work_units;
    int size = vm_ptr_->GetInnerVertexSize(_fid);
    //degree.resize(size);
        ...
        process_threads[tid] = std::thread([&]() {
          ...
          while (true) {
            ...
            while (src_iter != src_end) {
              ...
              vm_ptr_->GetGid(u_fid, *src_iter, ptr->src_); //fid,oid,gid
              // if (u_fid == _fid) {
              //     vid_t src_lid = vm_ptr_->GetLidFromGid(ptr->src_); //local vertex
              //     atomic_add(degree[src_lid], 1);
              // }
              vm_ptr_->GetGid(v_fid, *dst_iter, ptr->dst_);
              // if (v_fid == _fid) {
              //   vid_t des_lid = vm_ptr_->GetLidFromGid(ptr->dst_);
              //   atomic_add(degree[des_lid], 1);
              // } 
  ...
  void initMirrorInfo(std::shared_ptr<fragment_t> fragment) {
    ...
    std::thread send_thread([&]() {
      ...
      for (int i = 1; i < worker_num; ++i) {
        int dst_worker_id = (worker_id + i) % worker_num;
        fid_t dst_fid = comm_spec_.WorkerToFrag(dst_worker_id);
        auto range = fragment->OuterVertices(dst_fid);  //outer to each frages' local id range
        vid_t offsets[2];
        offsets[0] = range.begin().GetValue();
        offsets[1] = range.end().GetValue();
        MPI_Send(&offsets[0], sizeof(vid_t) * 2, MPI_CHAR, dst_worker_id, 0,
  ...
  void initWeightSum(std::shared_ptr<fragment_t> fragment,
                          std::vector<Edge<vid_t, edata_t>> processed_edges_) {
      ...
      int worker_num = comm_spec_.worker_num();
      vid_t gsource;
      oid_t source_id(FLAGS_php_source);
      vm_ptr_->GetGid(source_id, gsource);
     // LOG(INFO)<<"-------source :"<<gsource;
      
      fragment->calcFidBitWidth(fnum, id_mask, fid_offset);
      auto is_iv_gid = [this,&fid_offset](vid_t id) { return (id >> fid_offset) == comm_spec_.fid(); };
      for (auto& e : processed_edges_) {
        edata_t edata_ = e.edata();
        if (is_iv_gid(e.src_)) {
          if (edata_ == 0) {
            edata_ += 1;
          }
          if (e.dst() != gsource) {
            weight_sum[e.src()] += edata_;
            //LOG(INFO)<<comm_spec_.fid()<<" src: "<<e.src()<<" dst "<<e.dst()<<" count"<<weight_sum[e.src()]<<" e.data: "<<e.edata();
            }
          }
      }
      ...
      {
        std::thread recv_thread([&](){ 
          HashMap<vid_t, edata_t> tempsum;
          int src_worker_id = (worker_id + 1) % worker_num;
          while (src_worker_id != worker_id) {
              OutArchive arc;
              RecvArchive(arc, src_worker_id, comm_spec_.comm());
              arc >> tempsum;
              for (auto it : tempsum) {
        ...
        std::thread send_thread([&](){ 
          ...
          while (dst_worker_id != worker_id) {
            InArchive arc;
            arc << weight_sum;
            SendArchive(arc, dst_worker_id, comm_spec_.comm());
   ...
   // 只需要找出local master的outer fid即可,然后通信得到outer master的fid
  void initMasterFidList(std::shared_ptr<fragment_t> fragment) {
    int worker_id = comm_spec_.worker_id();
    int worker_num = comm_spec_.worker_num();
    fid_t fid_ = comm_spec_.fid();
    std::vector<HashMap<vid_t, std::vector<fid_t>>> fidlist;
    std::set<fid_t>set_fids;
    fidlist.resize(worker_num);
    auto range = fragment->InnerMasterVertices(); 

    for (auto v : range) {
      set_fids.clear();
      set_fids.insert(fid_); //insert local fid
      //auto oes = fragment->GetIncomingAdjList(v); //将master的远程顶点放置到了incoming数组里边.
      vid_t g = fragment->GetInnerVertexGid(v);
      std::vector<fid_t> tmpfid = fragment->getMasterInFid(v.GetValue());
      std::string s = std::to_string(g) +": fid:" +std::to_string(fid_);

      if (tmpfid.size() != 0) {
        for (fid_t it : tmpfid) {
          set_fids.insert(it);
          s +=  " fid:"+std::to_string(it);
        }
      }
     // LOG(INFO)<<"++++++"<<g<<"  fid: "<<fid_;
      // for (auto& n : oes) {
        
      //   typename fragment_t::vertex_t lid = n.neighbor;
        
      //   fid_t fid = fragment->GetFragId(lid);
      //   s +=  " fid:"+std::to_string(fid);
      //   //LOG(INFO)<<"###"<<g<<"  fid:"<<fid;
      //   set_fids.insert(fid);
      // }
      ...
      if (!set_fids.empty()) {
        for (auto tmpfid : set_fids) {
          fidlist[tmpfid][g].resize(set_fids.size());
          fidlist[tmpfid][g].assign(set_fids.begin(), set_fids.end());
        }
      }
    }
    ...
    {
      std::thread recv_thread([&](){ 
        HashMap<vid_t, std::vector<fid_t>> tempfidlist;
        int src_worker_id = (worker_id + 1) % worker_num;
        while (src_worker_id != worker_id) {
          for (fid_t fid = 0; fid < comm_spec_.fnum(); ++fid) {
            OutArchive arc;
            if (comm_spec_.FragToWorker(fid) != worker_id) {
              continue;
            }
            RecvArchive(arc, src_worker_id, comm_spec_.comm());
            arc >> tempfidlist;
            for (auto it : tempfidlist) {
              fidlist[fid][it.first].assign(it.second.begin(), it.second.end());
            }
            tempfidlist.clear();
          }
          src_worker_id = (src_worker_id + 1) % worker_num;
...
///////////////////////test///////////////////////
    ...
    for (auto i : fidlist[fid_]) {
      std::string s = "";
      for (auto f : i.second) {
         s += std::to_string(f) + " ";
      }
      if (i.first == 1073804385)
      LOG(INFO)<<"local_fid"<<fid_ <<" gid:"<<i.first <<" fid:"<< s;
    }
  
    ///////////////////////////////////////////
    fragment->SetUpMasterFidInfo(fidlist[fid_]);
 ...
 private:
  CommSpec comm_spec_;
  std::shared_ptr<vertex_map_t> vm_ptr_;
  HashMap<vid_t,edata_t>weight_sum;
  ...
  //std::vector<int> degree; //统计度数来确定multi-master顶点
  std::vector<std::vector<oid_t>> got_edges_src_;
  std::vector<std::vector<oid_t>> got_edges_dst_;
  std::vector<std::vector<edata_t>> got_edges_data_;

  std::vector<Edge<vid_t, edata_t>> processed_edges_;
  std::vector<Edge<vid_t, edata_t>> processed_mirror_edges_;
