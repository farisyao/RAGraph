 ...
 public:
  ...
  std::shared_ptr<fragment_t> LoadFragment(const std::string& efile,
    ...
    if (std::is_same<partitioner_t, SegmentedPartitioner<oid_t>>::value) {
      ...
      io_adaptor->Close();
      id_list.assign(id_set.begin(), id_set.end());
    }
    ...
    grape::EmptyType fake_data;
    {
      ...
      while (io_adaptor->ReadLine(line)) {
        ...
        basic_fragment_loader_.AddVertex(src, fake_data, true); //is src vertice,to cout outer degree
        basic_fragment_loader_.AddVertex(dst, fake_data, false);
